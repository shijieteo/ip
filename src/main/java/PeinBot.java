...
public class PeinBot {
            ...
            case "mark":
                try {
                    ...
                    printOutput(String.format("\tCongrats on completing the following task:\n\t %s",
                            this.taskList.get(taskListIndex)));
                }
            ...
            case "delete":
                try {
                    ...
                    printOutput("\t" + String.format("The following task was deleted: %s \n\t" +
                            "You now have %d tasks", deletedTask, this.taskList.size()));
                }
                catch (NumberFormatException numberFormatException) {
                    printOutput("\tPlease provide a number next time... :(");
                }
    ...
    private void listTasks() {
        ...
        int endLoopIndex = this.taskList.size();
        for (int index = 0; index < endLoopIndex; index++) {
            accumulatedTaskString += String.format("\t%d. %s", index + 1, this.taskList.get(index));
    ...
    private void markTasks(int index) {
        this.taskList.get(index).setIsDone(true);
    }

    private void unmarkTasks(int index) {
    ...
    private void addTasks(String userInput) {
        ...
        try {
            ...
            try {
                storage.writeData(this.taskList);
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
            printOutput("\t" + String.format("added: %s to your list of tasks\n\t" +
                    "You now have %d tasks", taskString, this.taskList.size()));
        }
    ...
    private Task createToDoTask(String[] taskSplit) throws IllegalArgumentException {
    ...
    private Task createEventTask(String[] taskSplit) throws IllegalArgumentException {
    ...
    private Task createDeadlineTask(String[] taskSplit) throws IllegalArgumentException {
        String dueDate = "";
        String taskDescription = "";
